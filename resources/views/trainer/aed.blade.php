...
<div x-data="{
    ...
    logShockButtonPress() {
        if (this.isOn && this.logCount < this.scenarioInstructionSelected.length) {
            console.log(this.logCount);
            console.log(this.scenarioInstructionSelected[this.logCount]);
            this.screen = this.scenarioInstructionSelected[this.logCount].params;
            this.showImage = false; // Ocultar la imagen de la pantalla
            this.logCount += 1;

            console.log(`Botón de descarga pulsado ${this.logCount} veces.`);
        }else if(this.isOn && this.logCount == this.scenarioInstructionSelected.length){
            this.logCount = 0;
    ...
    <div class="rounded-lg shadow-3xl w-full h-full flex flex-col justify-between items-center relative" style="width: var(--w-device); height: var(--h-device);">
        ...
        <div class="bg-neutral-700 w-[72%] aspect-square border-2 border-neutral-400 rounded-3xl inset-shadow-sm flex flex-col items-center justify-between p-4 relative">
            ...
            <div class="flex flex-col items-center justify-around w-full h-full">
                ...
                <button id="drawer-button" @click="drawerOpen = true" class="text-center font-bold text-xl text-neutral-400 uppercase border-2 border-black rounded-md py-1 px-4" type="button" data-drawer-target="drawer-scenarios" data-drawer-show="drawer-scenarios" aria-controls="drawer-scenarios">
                    ELEGIR ESCENARIO
                </button>
                <!-- Contenido de la pantalla (Texto dinámico) -->
                <div class="w-[80%] aspect-[1.21] flex items-center justify-center bg-black text-white text-2xl font-bold rounded-3xl">
                    <img x-show="showImage" src="{{ asset('images/screen.png') }}" alt="Screen Content" class="object-contain rounded-3xl max-w-full max-h-full">
    ...
    tabindex="-1" aria-labelledby="drawer-scenarios-label">
        ...
        <div class="flex justify-between items-center mb-2">
            ...
            aria-controls="drawer-scenarios" class="text-gray-400 bg-transparent w-8 h-8 hover:bg-gray-200 hover:text-gray-900 rounded-lg text-lg flex items-center justify-center">
